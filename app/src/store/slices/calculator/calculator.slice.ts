...
export const calculatorSlice = createSlice({
  ...
  reducers: {
    ...
    setOperation(state, action: PayloadAction<Operations>) {
      ...
      const sum = operate(state.operation, state.accumulatedValue, parseFloat(state.displayNumber));
    ...
    handleEqualsClick(state) {
      ...
      const sum = operate(state.operation, state.accumulatedValue, state.lastInput);
...
export const selectDisplayNumber = createSelector(calculatorState, ({ displayNumber }) => ({
  displayNumber
}));

export const { addNumber, setOperation, handleEqualsClick, addDecimalPoint, resetCalculator } =
