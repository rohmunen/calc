...
import { DropPositionLine } from '../../DragAndDrop/DropPositionLine';
    ...
    <DroppableArea
      ...
      {constructed.map((part) => (
        <div key={part.id} className="relative">
          ...
          {isOver && !draggingOver && part.id === constructed[constructed.length - 1].id && (
          ...
          <DraggableContainer
            ...
            onDoubleClickHandler={() => {
              setConstructed((prev) => prev.filter((currentPart) => currentPart.id !== part.id));
            }}>
            {part.element}
          </DraggableContainer>
        </div>
